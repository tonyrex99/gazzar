...
describe("/api/v1/product/:id", () => {
  test("getting a single result succeeds for authorized user", async () => {
    ...
    expect(res.body).toEqual(data);
    expect(res.status).toBe(200);
    expect(ProductService.get).toHaveBeenCalledWith("507f1f77bcf86cd799439011");
  });

  test("getting a single result fails for anonymous user", async () => {
    const req = supertest(app);
    const res = await req.get("/api/v1/product/507f1f77bcf86cd799439011");
    expect(res.status).toBe(401);
  });
  ...
  test("Product update", async () => {
    ...
    const res = await req
      .put(`/api/v1/product/507f1f77bcf86cd799439011`)
      .send(data)
      .set("Authorization", "token abc");
